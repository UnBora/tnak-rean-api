...
public class ClassMaterialController {
    ...
    public ClassMaterialController(ClassMaterialImp classMaterialService,
    ...
    ApiResponse<Boolean> createCourseByClass(
        ...
        try {
            if (user_id == 0) {
                ...
                Integer materialId = classMaterialRepository.creatNewCourse(user_id, title, description, createdDate,
    ...
    ApiResponse<Boolean> assignCourse(
            ...
            @RequestParam @Min(value = 1) int class_id) {
        boolean checkClassId = classMaterialRepository.findClassId(class_id);
        boolean checkClassIdANDMaterialIdInMD = classMaterialRepository.findClassIdANDMaterialIdInMD(class_id,
                class_material_id);
        try {
            if (checkClassId == false) {
                ...
                return ApiResponse.<Boolean>notFound("")
                        .setResponseMsg("Class Id: " + class_id + " and ClassMaterialId: " + class_material_id
                                + " already exist ");
            } else {
    ...
    ApiResponse<ClassMaterialUpdateTitleDesRequest> updateClassMaterial(
        ...
        try {
            // ClassMaterialResponse response =
            // classMaterialRepository.selectResponseAfterUpdate(classMaterialUpdateRequest.getId());
            ClassMaterialUpdateTitleDesRequest classMaterialUpdateTitleDesRequest1 = classMaterialServiceImp
                    .updateClassMaterial(classMaterialUpdateTitleDesRequest);
            if (classMaterialUpdateTitleDesRequest1 == null) {
                return ApiResponse
                        .<ClassMaterialUpdateTitleDesRequest>notFound(
                                ClassMaterialUpdateTitleDesRequest.class.getSimpleName())
                        .setResponseMsg("Can't update! ID: " + classMaterialUpdateTitleDesRequest.getClass_material_id()
                                + " doesn't exist");
            }
            classMaterialUpdateTitleDesRequest.setTitle(classMaterialUpdateTitleDesRequest.getTitle().trim());
            classMaterialUpdateTitleDesRequest
                    .setDescription(classMaterialUpdateTitleDesRequest.getDescription().trim());
            classMaterialServiceImp.updateClassMaterial(classMaterialUpdateTitleDesRequest);
            return ApiResponse
                    .<ClassMaterialUpdateTitleDesRequest>ok(ClassMaterialUpdateTitleDesRequest.class.getSimpleName())
    ...
    ApiResponse<ClassMaterialUpdateContentRequest> updateContent(
        ...
        try {
            // ClassMaterialResponse response =
            // classMaterialRepository.selectResponseAfterUpdate(classMaterialUpdateContentRequest.getId());
            ClassMaterialUpdateContentRequest response = classMaterialServiceImp
                    .updateContent(classMaterialUpdateContentRequest);
            if (response == null) {
                return ApiResponse
                        .<ClassMaterialUpdateContentRequest>notFound(
                                ClassMaterialUpdateContentRequest.class.getSimpleName())
                        .setResponseMsg("Can't update! ID: " + classMaterialUpdateContentRequest.getClass_material_id()
                                + " doesn't exist");
            }
            return ApiResponse
                    .<ClassMaterialUpdateContentRequest>ok(ClassMaterialUpdateContentRequest.class.getSimpleName())
                    .setResponseMsg(BaseMessage.Success.UPDATE_SUCCESS.getMessage())
                    .setData(response);
        } catch (Exception e) {
            return ApiResponse.setError(e.getMessage());
    ...
    ApiResponse<Boolean> deleteById(@RequestParam @Min(value = 1) Integer id) {
        try {
            ...
            if (checkMaterialId == false) {
                ...
                return ApiResponse.<Boolean>notFound("")
                        .setResponseMsg("Can't delete! classMaterialId: " + id
                                + " is still referenced from table class_materials_detail");
            } else {
    ...
    ApiResponse<List<ClassMaterialByTeacherResponse>> getAllClassMaterialByTeacherUserId() {
        try {
            Integer user_id = SecurityContextBean.getRequestingUser().getId();
            List<ClassMaterialByTeacherResponse> classMaterialResponses = classMaterialServiceImp
                    .getAllClassMaterialByTeacherUserId(user_id);
            if (classMaterialResponses.isEmpty()) {
                return ApiResponse
                        .<List<ClassMaterialByTeacherResponse>>notFound(
                                ClassMaterialByTeacherResponse.class.getSimpleName())
                        .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
                        .setData(classMaterialResponses);
            }
            return ApiResponse
                    .<List<ClassMaterialByTeacherResponse>>ok(ClassMaterialByTeacherResponse.class.getSimpleName())
                    .setResponseMsg(BaseMessage.Success.SELECT_ALL_RECORD_SUCCESS.getMessage())
                    .setData(classMaterialResponses);
        } catch (Exception e) {
            return ApiResponse.setError(e.getMessage());
    ...
    ApiResponse<List<ClassMaterialByTeacherResponse>> getCourseMaterialByFolderId(
        ...
        try {
            Integer user_id = SecurityContextBean.getRequestingUser().getId();
            List<ClassMaterialByTeacherResponse> responses = classMaterialServiceImp
                    .getCourseMaterialByTFolderId(folder_id, user_id);
    ...
    ApiResponse<List<ClassMaterialByClassIdAndClassroomIdResponse>> getCourseMaterialByFolderIdInClass(
        ...
        try {
            Integer user_id = SecurityContextBean.getRequestingUser().getId();
            List<ClassMaterialByClassIdAndClassroomIdResponse> responses = classMaterialServiceImp
                    .getCourseMaterialByFolderIdInClass(folder_id, class_id, classroom_id);
            if (responses.isEmpty()) {
                return ApiResponse
                        .<List<ClassMaterialByClassIdAndClassroomIdResponse>>notFound("Course Material By FolderId")
                        .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
                        .setData(responses);
            }
    ...
    public ApiResponse<List<CourseByStudentIdResponse>> getCourseByStudentId() {
    ...
    ApiResponse<?> setMaterialToFolder(
        ...
        boolean checkFolderIdAndMaterialIdInMF = classMaterialRepository.findFolderIdAndMaterialIdInMF(folder_id,
                material_id);
        ...
        try {
            if (checkMaterialId == false) {
                ...
                return ApiResponse.<Boolean>notFound("")
                        .setResponseMsg(
                                "folder_id:" + folder_id + " And material_id:" + material_id + " already exist");
            }
    ...
    // ApiResponse<List<ClassMaterialResponse>>
    // getClassMaterialByTeacherUserIdAndMaterialType(
    // @RequestParam @Min(value = 1) Integer teacher_id,
    // @RequestParam @Min(value = 1) Integer class_materials_type_id
    // ) {
    // try {
    // List<ClassMaterialResponse> classMaterialResponses =
    // classMaterialServiceImp.getClassMaterialByCreatedByAndMaterialType(teacher_id,
    // class_materials_type_id);
    // if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialResponse>>notFound(ClassMaterialResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialResponses);
    // }
    // return
    // ApiResponse.<List<ClassMaterialResponse>>ok(ClassMaterialResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ALL_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialResponses);
    // } catch (Exception e) {
    // return ApiResponse.setError(e.getMessage());
    ...
    // try {
    // List<ClassMaterialResponse> classMaterialResponses =
    // classMaterialServiceImp.getClassMaterialByMaterialTypeId(class_materials_type_id);
    // if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialResponse>>notFound(ClassMaterialResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    ...
    ApiResponse<List<ClassMaterialByClassIdAndClassroomIdResponse>> getByClassIdAndClassroomId(
            ...
            @RequestParam @Min(value = 1) Integer classroom_id) {
        try {
            List<ClassMaterialByClassIdAndClassroomIdResponse> classMaterialByClassIdAndClassroomIdResponses = classMaterialServiceImp
                    .getByClassIdAndClassroomId(class_id, classroom_id);
            if (classMaterialByClassIdAndClassroomIdResponses.isEmpty()) {
                return ApiResponse
                        .<List<ClassMaterialByClassIdAndClassroomIdResponse>>notFound(
                                ClassMaterialByClassIdAndClassroomIdResponse.class.getSimpleName())
                        .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
                        .setData(classMaterialByClassIdAndClassroomIdResponses);
            }
            return ApiResponse
                    .<List<ClassMaterialByClassIdAndClassroomIdResponse>>ok(
                            ClassMaterialByClassIdAndClassroomIdResponse.class.getSimpleName())
                    .setResponseMsg(BaseMessage.Success.SELECT_ALL_RECORD_SUCCESS.getMessage())
                    .setData(classMaterialByClassIdAndClassroomIdResponses);
        } catch (Exception e) {
    ...
    ApiResponse<Boolean> deleteByCreatedByAndMaterialId(
        ...
        Integer teacher_id = SecurityContextBean.getRequestingUser().getId();
        Boolean checkMaterialId = classMaterialRepository.findMaterialId(material_id);
        Boolean checkMaterialIdWithCreatedBy = classMaterialRepository.findMaterialIdWithCreatedBy(material_id,
                teacher_id);
    ...
    // ApiResponse<List<ClassMaterialByClassIdResponse>> getByClassId(@RequestParam
    // @Min(value = 1) Integer class_id) {
    // try {
    // List<ClassMaterialByClassIdResponse> classMaterialByClassIdResponses =
    // classMaterialServiceImp.getByClassId(class_id);
    // if (classMaterialByClassIdResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialByClassIdResponse>>notFound(ClassMaterialByClassIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialByClassIdResponses);
    // }
    // return
    // ApiResponse.<List<ClassMaterialByClassIdResponse>>ok(ClassMaterialByClassIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ONE_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialByClassIdResponses);
    // } catch (Exception e) {
    // return ApiResponse.setError(e.getMessage());
    ...
    // ApiResponse<List<ClassMaterialByClassIdAndMaterialTypeResponse>>
    // getByMaterialTypeAndClassId(
    // @RequestParam @Min(value = 1) Integer class_materials_type_id,
    // @RequestParam @Min(value = 1) Integer class_id
    // ) {
    // try {
    // List<ClassMaterialByClassIdAndMaterialTypeResponse> classMaterialResponses =
    // classMaterialServiceImp.getByMaterialTypeAndClassId(class_materials_type_id,
    // class_id);
    // if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialByClassIdAndMaterialTypeResponse>>notFound(ClassMaterialByClassIdAndMaterialTypeResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialResponses);
    // }
    // return
    // ApiResponse.<List<ClassMaterialByClassIdAndMaterialTypeResponse>>ok(ClassMaterialByClassIdAndMaterialTypeResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ALL_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialResponses);
    // } catch (Exception e) {
    // return ApiResponse.setError(e.getMessage());
    ...
    // ApiResponse<List<ClassMaterialByStudentIdResponse>>
    // getByStudentId(@RequestParam @Min(value = 1) Integer student_user_id) throws
    // IllegalStateException {
    // try {
    // List<ClassMaterialByStudentIdResponse> classMaterialResponses =
    // classMaterialServiceImp.getByStudentId(student_user_id);
    // if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialByStudentIdResponse>>notFound(ClassMaterialByStudentIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialResponses);
    // }
    // return
    // ApiResponse.<List<ClassMaterialByStudentIdResponse>>ok(ClassMaterialByStudentIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ONE_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialResponses);
    // } catch (Exception e) {
    // return ApiResponse.setError(e.getMessage());
    ...
    // ApiResponse<List<ClassMaterialByStudentIdClassIdAndClassroomIdResponse>>
    // getByUserClassClassroom(
    // @RequestParam @Min(value = 1) Integer student_user_id,
    // @RequestParam @Min(value = 1) Integer class_id,
    // @RequestParam @Min(value = 1) Integer classroom_id
    // ) {
    // try {
    // List<ClassMaterialByStudentIdClassIdAndClassroomIdResponse>
    // classMaterialResponses =
    // classMaterialServiceImp.getByUserClassClassroom(student_user_id, class_id,
    // classroom_id);
    // if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialByStudentIdClassIdAndClassroomIdResponse>>notFound(ClassMaterialResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialResponses);
    // }
    // return
    // ApiResponse.<List<ClassMaterialByStudentIdClassIdAndClassroomIdResponse>>ok(ClassMaterialResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ONE_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialResponses);
    // } catch (Exception e) {
    // return ApiResponse.setError(e.getMessage());
    ...
    // ApiResponse<List<ClassMaterialByTeacherIdAndClassIdResponse>>
    // getByClassIdAndTeacherId(
    // @RequestParam @Min(value = 1) Integer class_id
    // ) {
    // try {
    // Integer teacher_id = SecurityContextBean.getRequestingUser().getId();
    // List<ClassMaterialByTeacherIdAndClassIdResponse> classMaterialResponses =
    // classMaterialServiceImp.getByClassIdAndTeacherId(teacher_id, class_id);
    // if (teacher_id == 0){
    // return ApiResponse.unAuthorized("unAuthorized");
    // } else if (classMaterialResponses.isEmpty()) {
    // return
    // ApiResponse.<List<ClassMaterialByTeacherIdAndClassIdResponse>>notFound(ClassMaterialByTeacherIdAndClassIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Error.SELECT_ERROR.getMessage())
    // .setData(classMaterialResponses);
    // } else {
    // return
    // ApiResponse.<List<ClassMaterialByTeacherIdAndClassIdResponse>>ok(ClassMaterialByTeacherIdAndClassIdResponse.class.getSimpleName())
    // .setResponseMsg(BaseMessage.Success.SELECT_ALL_RECORD_SUCCESS.getMessage())
    // .setData(classMaterialResponses);
    // }
    // } catch (Exception e) {

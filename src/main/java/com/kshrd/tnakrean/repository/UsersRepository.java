...
public interface UsersRepository {
    ...
    //    Update profile
    @Update("UPDATE users SET name=#{name}, username=#{username},email=#{email}, gender=#{gender}, img=#{img} WHERE id = #{user_id}")
    void updateProfile(Integer user_id, String name, String username, String email, String gender, String img);

    @Select("select exists (select email from users where email =#{email})")
    Boolean checkEmailExist(String email);

    //    Select name
    @Select("select img from users where id =#{id}")
    ...
    @Select("SELECT n.received_id, (SELECT username from users where id=n.received_id) as received_name, " +
            "(SELECT users.img from users where id=n.received_id) as received_img, " +
            "n.sender_id,(SELECT username from users where id=n.sender_id) as sender_name," +
            "(SELECT users.img from users where id=n.sender_id) sender_img, n.received_class_id as received_class_id, " +
            " (SELECT class_name from class where id= n.received_class_id) as class_name, nt.type, nt.title, nt.action_on, " +
            "n.received_date FROM notification_detail nd " +
